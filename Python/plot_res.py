...
for step in data.keys():
    ...
    if 'n' in step_str:
        ...
        numeric_value = float(step_str.replace('m', '')) * 1e-3  # Convert microseconds to seconds
    else:
...
ini = 590  # Assuming `ini` is the start index for slicing `values`

for i, (step, values) in enumerate(list(data.items())):
    color = viridis(norm(step_values_seconds[i]))  # Get color based on normalized log scale
    maxq = np.max(values['V(q)'][ini:])
    minq = np.min(values['V(q)'][ini:])
    center = (maxq-minq)/2+minq
    
    ax.plot(
        values['V(in)'][ini:], 
        values['V(q)'][ini:]-center, 
        linewidth=2,
...
cbar.set_label(r'$Resistance$ ($\Omega$)', labelpad=25, rotation=270, fontsize=15)
cbar.ax.tick_params(labelsize=12) 
# Define major ticks at key points only to keep it simple and readable
major_ticks = [1e-3, 10e-3, 100e-3, 1]
